...
def sweep(
    ...
    return compute_squashing(field, _seeds(colatitude_deg), rtol=rtol, show_progress=True)
...
def seed_invariance(field: PfssDipoleField, *, colatitude_deg: float = 30.0) -> tuple[float, float]:
    ...
    line = compute_squashing(
        field, _seeds([colatitude_deg]), rtol=1e-6, store_path=True, show_progress=False
    )
    ...
    reseeded = compute_squashing(field, sample, rtol=1e-6, show_progress=False)
...
def _print_comparison(field: PfssDipoleField) -> None:
    ...
    result = compute_squashing(field, _seeds(reference_deg), rtol=1e-6, show_progress=False)
    ...
    print(f"\n  seed-invariance (theta=30 line): mean Q_perp = {mean_q:.6f}, range {spread:.1e}")
...
def _make_figure(
    ...
    glob.set_yscale("log")
    glob.set_xlabel("colatitude theta [deg]")
    glob.set_ylabel("Q_perp")
    ...
    zoom.set_ylim(1.9, 200.0)
    zoom.set_xlabel("colatitude theta [deg]")
    zoom.set_ylabel("Q_perp")
    zoom.set_title(f"zoom on separatrix theta_SL = {theta_sl:.2f} deg")
    ...
    resid.set_xlabel("colatitude theta [deg]")
    resid.set_ylabel("|Q_engine / Q_theory - 1|")
    resid.set_title("relative error vs theory")
...
def main() -> None:
    ...
    # Dense sweep for the figure, skipping a small margin around the separatrices (seeds there run
    # toward the cusp null and are deliberately unclassified, as in the tracer/squashing gates).
    theta_sl = np.degrees(field.separatrix_colatitude(R_SEED))
    colatitude_deg = np.linspace(2.0, 178.0, 705)
    clear_of_separatrix = (np.abs(colatitude_deg - theta_sl) > 0.4) & (
        np.abs(colatitude_deg - (180.0 - theta_sl)) > 0.4
    )
